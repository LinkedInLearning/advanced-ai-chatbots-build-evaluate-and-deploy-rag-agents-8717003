...
# object. Reading result["structured_response"] directly then returns None,
# and the next line (response.answer) crashes with a confusing error. This
# helper catches that case and falls back to the model's plain-text reply,
# so the app never hard-crashes in front of a learner.
# ---------------------------------------------------------------------------
...
def safe_answer(result) -> AnswerWithCitations:
    ...
    # Fallback: use the last message's text, with no citations.
